...
def docs_dir(mocker: MockerFixture, tmp_path: Path) -> Path:
    ...
    (tmp_path / "excluded_titles.yaml").write_text("[]\n", encoding="utf-8")
    return output_dir
...
def test_main_writes_the_disclaimers_and_attribution_in_the_footer(
    docs_dir: Path, write_yaml: WriteYaml, title_row: dict[str, object], sku_row: dict[str, object]
) -> None:
    ...
    html = (docs_dir / "index.html").read_text(encoding="utf-8")
    assert "not affiliated with or endorsed by Nintendo" in html
    assert "Mistakes are possible" in html
    assert '<a href="https://www.igdb.com">IGDB</a>' in html
    assert "issues/new?template=correction.yml" in html
...
def test_main_keeps_the_anchor_of_a_title_merged_into_another(
    ...
    merged = {
        ...
        "former_title_id": "example-game-deluxe-edition",
        "merged_into": "example-game",
    }
    write_yaml("excluded_titles.yaml", [merged])
