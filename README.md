...
- `data/titles.yaml` — title metadata, generated from IGDB via the seeds above.
...
3. Import metadata from IGDB. By default this **skips titles already in `titles.yaml`**, so it's safe to
   re-run after a partial or failed run — it only fetches what's still missing:

```bash
uv run --env-file .env python -m scripts.import_titles              # import everything pending
uv run --env-file .env python -m scripts.import_titles --count 20   # import only the next 20 pending
...
4. Add SKUs for the new titles to `data/skus.yaml` and validate.
...
4. Add its SKUs to `data/skus.yaml` and validate.
...
Checks `title_seeds.yaml`, `titles.yaml` and `skus.yaml` for schema and integrity errors, logs warnings, and reports fill rate. Exit code is non-zero on errors.
...
Renders `docs/index.html` (ES/EN, paginated) from `titles.yaml` and `skus.yaml`. Commit the regenerated `docs/` to publish.
